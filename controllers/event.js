...
const getAllEvent = async (req, res, eventData) => {
  ...
  //   const loggeduser = await req.params._id;
  //   const postid = req.params.pid;
  console.log("Hellooooooooooooooooooo", loggedUser);
  let result1 = await userData.findOne({ _id: loggedUser });
  ...
  if (result1.u_phone == null && result1.u_city == null) {
    ...
    let Number_of_event_created = await eventData.find({
      e_org_id: result1._id,
    });
    let no_event = Number_of_event_created.length;
    // eventcreated end here

    //joined event start here
    let joinedEvents = result2.filter((event) =>
      event.e_joinies.includes(result1._id)
    );
...
const joinEvents = async (req, res, eventData) => {
  ...
  if (!flag) {
    let result = await eventData.updateOne(
      { _id: req.params.id },
      { $push: { e_joinies: req.session.user._id } }
    );
...
const getComment = async (req, res, eventData) => {
  try {
    ...
    //joined event start here
    let joinedEvents = events.filter((event) =>
