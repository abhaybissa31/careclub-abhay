...
const chatFn = require("./controllers/chatfnnew");
...
function routes(app) {
  ...
  app.post(
    "/create-event",
    upload.single("e_image"),
    eventDetails.createEvent(eventData, storage)
  ); //handeling event
  app.get("/events", islogin, (req, res) =>
    eventDetails.getAllEvent(req, res, eventData)
  ); //getting all events
  app.get("/join-event/:id", (req, res) =>
    eventDetails.joinEvents(req, res, eventData)
  );
  app.get("/joined-users/:id", (req, res) =>
    eventDetails.joinedUsers(req, res, eventData)
  );

  app.get("/joined-events", (req, res) =>
    eventDetails.getJoinedEventsFile(req, res, eventData)
  );
  app.get("/organised-events", (req, res) =>
    eventDetails.organisedEvents(req, res, eventData)
  );
  // app.post('/joined-events',eventDetails.JoinedEvents(eventData))

  //written by abishek start here
  app.get("/like/:id/:pid", (req, res) =>
    eventDetails.likeEvent(req, res, eventData)
  );
  ...
  // })
  app.post("/comment/:id/:usrid", (req, res) =>
    eventDetails.commentEvent(req, res, eventData)
  );
  app.get("/getcomment/:eventId", (req, res) =>
    eventDetails.getComment(req, res, eventData)
  );

  app.get("/trending", islogin, showTrendingHashtag, (req, res) =>
    trendingDetails.getTrendFile(req, res)
  );
  ...
  app.get("/events/filter/:filter", (req, res) =>
    eventDetails.filterevents(req, res, eventData)
  );

  app.get("/chat", (req, res) => chatFn.getInitialChats(req, res));
  app.post("/chat", (req, res) => chatFn.postMessages(req, res));
  app.post("/chat/:chatName", (req, res) =>
    chatFn.getRecentMessages(req, res, req.params.chatName)
  );
  ...
  app.get("/profile", islogin, async (req, res) => {
    ...
    let joinedEvents = tmp.filter((event) =>
      event.e_joinies.includes(user._id)
    );
  ...
  app.get("/click", (req, res) => {
    ...
    res.send(
      '<a href="https://careclub-yfue.onrender.com/auth/google?scope=email"> Sign up with google</a> '
    );
  });
  //
    ...
    if (!errors) {
      ...
      if (req.file) {
        ...
        const storageRef = ref(
          storage,
          `files/${
            +Math.floor(Math.random() * 1000 + 1) + "-" + req.file.originalname
          }`
        );
        const metaData = {
